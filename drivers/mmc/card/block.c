				      ...
				      struct mmc_blk_data *md)
{
	...
	if (mmc_card_mmc(card)) {
		...
		/*zte add for samsung 4G emmc RPMB FAIL problem*/
		if ((card->quirks & MMC_QUIRK_INAND_FLUSH_CACHE)
			&& (part_config & 0x7) == EXT_CSD_PART_CONFIG_ACC_RPMB) {
			ret = mmc_flush_cache(card);
			pr_info("%s: mmc_flush_cache for RPMB->USER,part_config::0x%x,(ret=%d)\n",
				__func__, part_config, ret);
		}
...
static const struct mmc_fixup blk_fixups[] =
{
	...
	MMC_FIXUP("VZL00M", CID_MANFID_SAMSUNG, CID_OEMID_ANY, add_quirk_mmc,
		  MMC_QUIRK_SEC_ERASE_TRIM_BROKEN),
	MMC_FIXUP("FJ25AB", CID_MANFID_SAMSUNG, CID_OEMID_ANY, add_quirk_mmc,
		  MMC_QUIRK_INAND_FLUSH_CACHE),
	END_FIXUP
};
